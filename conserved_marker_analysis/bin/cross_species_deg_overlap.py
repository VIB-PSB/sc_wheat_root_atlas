            ...
            for genes_or_group in ["genes", "groups"]:
                ...
                for cluster in correspondance_dict[tissue][species]:
                    if cluster == "": 
                        ...
                        if cluster in cluster2genes_and_groups:
    ...
    for tissue, species_to_clusters in correspondance_dict.items():
        ...
        for species, df in species2df_dict.items():
            ...
            if clusters != [""]:
                ...
                # Calculate the ranks of that dataframe based on adjusted p-value (and log2FC to break ties)
                df_best_degs_top["Rank"] = list(df_best_degs_top.sort_values(["p_val_adj", "avg_log2FC"], ascending=(True, False))\
                                                                .reset_index()\
                                                                .index+1
                                               )
